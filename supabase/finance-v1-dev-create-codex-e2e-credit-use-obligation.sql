...
-- Pravi idempotentno avgustovsko zaduzenje za proveru koriscenja kredita.
-- Ograniceno je na jednu jasno oznacenu CODEX E2E email adresu.

do $$
declare
  v_email constant text := 'codex.e2e.member.001@example.com';
  v_month constant date := date '2026-08-01';
...
begin
  ...
  insert into public.financial_obligations (
    ...
    v_member.society_id, 'MEMBERSHIP_FEE', v_member.person_id, v_member.id,
    v_month, 'CODEX E2E test koriscenja kredita 08/2026', 100, 100,
    v_currency, date '2026-08-15', 'OPEN'
  )
  ...
  insert into public.membership_fee_assessments (
    society_id, society_member_id, assessment_month, result, obligation_id,
    assessed_amount, currency, process_source, note
  ) values (
    v_member.society_id, v_member.id, v_month, 'CHARGED', v_obligation_id,
    100, v_currency, 'RECHECK',
    'CODEX E2E kontrolisano zaduzenje za proveru koriscenja kredita'
  )
  ...
  raise notice 'CODEX E2E kredit test spreman. obligation_id=%', v_obligation_id;
end;
$$;
